  ...
  return (
    <section className="py-12 sm:py-20 max-w-7xl mx-auto px-4 sm:px-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-12 gap-4">
        ...
        <Link
          to="/stylist"
          className="text-xs uppercase tracking-widest text-stone-400 hover:text-stone-900 transition-colors flex items-center gap-1"
        >
          Открыть стилиста <ArrowRight size={12} />
        </Link>
      </div>
          ...
          <button
            ...
            className={`text-xs uppercase tracking-widest px-4 sm:px-5 py-2.5 border transition-colors whitespace-nowrap shrink-0 ${
              activeScenario === scenario
                ...
                : "border-stone-300 text-stone-500 hover:border-stone-900 hover:text-stone-900 active:bg-stone-50"
            }`}
          >
      ...
      {loading ? (
        <div className="py-16 sm:py-20 text-center">
          ...
          <p className="font-serif italic text-stone-400 text-base sm:text-lg">Подбираем образы…</p>
        </div>
      ) : error ? (
        <div className="py-16 sm:py-20 text-center">
          <p className="font-serif italic text-stone-500 text-base sm:text-lg">{error}</p>
          <p className="text-xs uppercase tracking-widest text-stone-400 mt-3 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 sm:gap-16">
          {looks.map((look, index) => (
            <OutfitCard
              ...
              outfit={look}
              title={`${getScenarioLabel(activeScenario)} · образ ${index + 1}`}
            />
          ))}
        </div>
      ) : (
        <div className="py-16 text-center">
          <p className="font-serif italic text-stone-400 text-base">
            Для этого сценария пока не найдено комплектов.
          </p>
        </div>
      )}
    </section>
